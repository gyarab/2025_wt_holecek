...
const SALT_LEN = 16;
const IV_LEN = 12;
const IDLE_TIMEOUT_MS = 5 * 60 * 1000; // 5 minut auto-lock
...
let idleTimer = null; 
let editingIndex = -1; // <--- NOVÉ: Sleduje, které heslo zrovna upravujeme (-1 = žádné)
...
// --- PŘIDÁVÁNÍ A EDITACE HESLA (UPRAVENO) ---
async function handleAddPassword() {
    if (sName.value && sPass.value) {
        // Změna tlačítka na "Pracuji..."
        ...
        if (editingIndex >= 0) {
            // REŽIM ÚPRAVY: Přepíšeme existující
            decryptedVault[editingIndex] = newItem;
            editingIndex = -1; // Reset indexu
            showToast("Změny uloženy ✅");
        } else {
            // REŽIM NOVÉHO: Přidáme na konec
            decryptedVault.push(newItem);
            showToast("Uloženo ✅");
        }

        // Reset UI zpět do režimu "Přidat"
        btnAdd.innerHTML = "<span>+</span> Uložit heslo";
        btnAdd.classList.remove("btn-secondary"); // Pokud tam byla barva pro editaci
        ...
        try {
            ...
            showError(e.message); 
            btnAdd.innerHTML = originalText; // Vrátit text při chybě
        }
    } else {
...
// Funkce pro zahájení editace (volaná tlačítkem tužky)
function startEditing(index) {
    const item = decryptedVault[index];
    
    // 1. Vyplnit formulář daty
    sName.value = item.site;
    sUser.value = item.user;
    sPass.value = item.pass;

    // 2. Nastavit globální index
    editingIndex = index;

    // 3. Změnit vzhled tlačítka
    btnAdd.innerHTML = "<span>💾</span> Uložit změny";
    btnAdd.classList.add("btn-secondary"); // Aby bylo vizuálně jiné
    
    // 4. Scrollovat nahoru k formuláři
    document.querySelector(".add-new-section").scrollIntoView({ behavior: "smooth" });
    
    // 5. Focus na první pole
    sName.focus();
}
...
function renderList(filterText = "") {
    ...
    itemsToShow.forEach((item, index) => {
        // Získáme reálný index (kvůli filtrování)
        ...
        // ZDE JE PŘIDÁNO TLAČÍTKO EDITACE (tužka)
        div.innerHTML = `
        ...
        document.getElementById(`del-${realIndex}`).addEventListener("click", async () => {
            if(confirm("Smazat?")) {
                ...
                // Pokud jsme zrovna editovali toto heslo, zrušit editaci
