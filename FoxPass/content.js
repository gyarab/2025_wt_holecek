const ICON_URL = browser.runtime.getURL("icon.jpg");// Ikona rozšíření
let lastClickedInput = null; 

browser.runtime.onMessage.addListener((message) => {// Poslouchá data k vyplnění
...
function scanInputs() {
    ...
    inputs.forEach(input => { //Ignorovat skrytá tlačítka
        if (input.type === "hidden" || input.type === "submit" || input.type === "button" || input.style.display === "none") return;
        if (input.offsetParent === null) return;//Pokud heslo nebo uživatel
...
function attachIcon(targetInput) {//Funkce přidávající ikonu vedle inputu
